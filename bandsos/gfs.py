...
functionality is particularly useful when unique segments from previous gfs forecasts are needed to be combined
to get a continuous sflux file.

.. warning::
    This module does not provide any classes currently and the functions in this module are standalone.  As the
    functionalities are not yet formalized into a data class. However, in future version this functions might be 
    deprecated and merged into a data class.
...
def _combine_gfs_cycles_temp(cycles: pd.DataFrame, fname: str, temp_dir: str, temp_name: str):
    """
    Internal combining procedure used by combine_gfs_cycles
    """
    logging.info(f'Started preparing {len(cycles.index) - 1} previous gfs cycles.')
    for i in range(len(cycles.index) - 1):
        with xr.open_dataset(cycles.fpath.values[i]) as ds:
            dt = pd.to_timedelta(ds.time.diff(dim='time'))[0]
            start_time = cycles.start_date.values[i]
            end_time = cycles.start_date.values[i + 1] - dt  # to avoid overlapping with the next
    ...
    with xr.open_dataset(cycles.fpath.values[-1]) as ds:
        ds.to_netcdf(
...
def combine_gfs_cycles(cycles: pd.DataFrame, fname: str):
    ...
    try:
        ...
        logging.fatal(f"Cycles {cycles}can not be combined due to {e}")
        raise e
    finally:
