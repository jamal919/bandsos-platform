...
license at our [github repository](https://github.com/jamal919/bandsos-platform). The source-code can be obtained
through `git` using `git clone https://github.com/jamal919/bandsos-platform`, or directly downloading from the
repository.
...
To install the toolbox system-wide in linux, install `python`, `numpy`, `scipy`, `gdal-python`, `gdal`, `proj`,
`xarray`,
...
Conda is a python environment management system, which can install not only python-only programs (e.g., environments
created
with venv) but also supplementary/required binary programs.

A brief installation procedure can be found here - https://docs.anaconda.com/anaconda/install/index.html. The relevant
conda environment can be created using the following command:

`conda install -y -n base -c conda-forge python=3.13 numpy pandas netcdf4 xarray dask gdal cartopy rasterio rioxarray utide cmocean f90nml ghapi f90nml herbie-data`

## Docker container

Docker is a containerization technique to isolate the necessary dependency-components for running a software into an
isolated
environment. Docker has the added benefit the ability to run in windows, linux, mac, or other ported platforms typically
without any change in the containerized system. It also allows easy deployment into cloud computing systems, or large
...
- ```fluxes```: root folder for flux directory, which contains ```gfs```, ```hwrf```, ```jtwc```, ```discharge```
  folders
    - ```gfs```: 0.25 degree global GFS forecast for the target region
...
- ```forecasts```: folder containing each forecast, for data saving the script might be set to delete forecast once
  published
...
- ```bandsos.env``` : The environment file which contains the environment variable to be used with Github pages. !!!
  Important
  to remember that this file should only be used in production, not for personal cases !!!

To run the bandsos in a automated forecasting, run the following command after ```cd``` to the directory where
```config```,
